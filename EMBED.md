...
- **Click tracking** — every "Call Now" tap is recorded in the
  ...
  `user_id` is empty for visitors who aren't logged into Robinhood).
...
2. The popup checks eligibility against the Robinhood server (DigitalOcean)
   using the **visitor's** IP (GeoIP → US/CA) and the current Pacific time.
3. If eligible, the iframe posts `{ type: "robinhood-specialist-popup", open: true }`
...
Clicks from your website and from inside the Robinhood app all land in the same
table.
...
- Do not add `X-Frame-Options`/`frame-ancestors` restrictions to the Robinhood
